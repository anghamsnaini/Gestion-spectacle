...
BEGIN
    IF idspec_exist(id_spec) THEN
        ...
        WHERE idspec = id_spec;
    dbms_output.put_line('modification effectué avec succée !');

    ELSE
        raise_application_error(-20002, 'Aucun Spéctacle avec cet id !');
    END IF;
    commit;
END;
**********Test****************
...
BEGIN                 
     IF spec_valid(date_spec, h_d, D, id_l) THEN  
        ...
        ELSE
            raise_application_error(-20032, 'Lieu n''est pas disponible pendant à cet horaire!');
        END IF;    
END ajout_spec;
